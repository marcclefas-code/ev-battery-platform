...
async def list_property_definitions(_: dict = Depends(ReadOnly())):
...
async def list_wave_policies(_: dict = Depends(ReadOnly())):
...
@router.get("/brands")
async def list_brands(_: dict = Depends(ReadOnly())):
    return {
        "brands": [
            ...
            {"name": "JLR", "code": "jlr", "source_domain": "topix.ourworld.eu", "status": "planned"},
