...
@router.get("/makes")
async def list_makes(_: dict = Depends(read_only)):
...
async def list_models(make: str, _: dict = Depends(read_only)):
    async with get_db_session() as session:
        result = await session.execute(
...
async def list_vehicle_batteries(make: str, model: str, _: dict = Depends(read_only)):
    async with get_db_session() as session:
        result = await session.execute(
